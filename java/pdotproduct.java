class pdotproduct extends Thread {
	...
	public static void main(String[] args) {
		...
		if (args.length != 1) {
			System.out.println("Usage:java PDotProduct <numthreads>");
			System.exit(0);
		}
		...
		if (BAD_FS) {
			if (numthreads == 1) {
				System.out.println("# PDotProduct: GOOD : N=" + N
		...
		if (BAD_MA) {
			System.out.println("# PDotProduct: BAD_MA : N=" + N + " : Threads="
					+ numthreads + " : CompTime(ms)=" + comp_time
					+ " : CompTime/TotalTime=" + (comp_time * 100 / total_time)
					+ "%");
		}
			...
			if (BAD_MA) {
				...
				for (k = 0; k < STRIDE; k++) { // bad memory access (strided)
					for (i = 0; i < len / STRIDE; i++) {
						n = start + (i * STRIDE + k);
						localSum[tId] += v1[n] * v2[n];
					}
				}
				if ((len / STRIDE) * STRIDE != len) { // do the remainder, if
														// any
