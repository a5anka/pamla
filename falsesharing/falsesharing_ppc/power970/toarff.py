...
attrib=["r1426","r0813","r50cb","r0704","r5001","r0705","r0935","r504b","r1800","r2837","r442d","r0800","r0925","r0320","instructions"]
skip_attrib=['instructions']
divisor_key='instructions'
#-----------------------------------------------------
	...
	while line :					# data line
		...
		if not re.search("<not counted>", line):  
			if line.count(':') == 1:
				...
				[value, key, stddev] = re.split(":", line)
				print("Value : "+value+" Key : "+key+" Std Dev : "+stddev)
					# or 3 fields "value:key:stddev"
			else :
				sys.exit('Error in input file: Line %d' %(line_num))
			#value = int(value)
		...
		for k in attrib:
			...
			if (k not in skip_attrib):
				if (ratio_mode == 0):
					...
					#print("%.0f, "%(float(strvalue)*100000000000))
					outf.write("%.0f, "%(float(strvalue)*100000000000))
					#outf.write(strvalue+', ')
					#print strvalue,', '	
		outf.write(mode+'\n')
		#print mode
